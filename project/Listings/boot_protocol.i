#line 1 "..\\Protocol\\boot_protocol.c"
...
#line 2 "..\\Protocol\\boot_protocol.c"
...
#line 4 "..\\Protocol\\boot_protocol.c"
                                                   ...
                                                   boot_proto_frame_t *frame)
{
    ...
    if ((len != expected_len) ||
        (proto_read_be16(&bytes[0]) != 0xA5B6U) ||
        (bytes[8] != 0x02U) ||
        (proto_read_be16(&bytes[expected_len - 2U]) != 0xB6A5U) ||
                           ...
                           uint8_t content_len)
{
    ...
    packet[pos++] = (uint8_t)(0xA5B6U >> 8);
    packet[pos++] = (uint8_t)0xA5B6U;
    ...
    packet[pos++] = (uint8_t)(0xB6A5U >> 8);
    packet[pos++] = (uint8_t)0xB6A5U;
...
void boot_proto_send_ok(uint16_t device_id, uint16_t command)
{
    ...
    boot_proto_send_frame(device_id, 0x02U, command, &ok, 1U);
}

void boot_proto_send_error(uint16_t device_id, uint16_t command)
{
    boot_proto_send_frame(device_id, 0xFFU, command, 0, 0U);
}

void boot_proto_send_heartbeat(uint16_t device_id)
{
    boot_proto_send_frame(device_id, 0x05U,
                          0x8888U, 0, 0U);
}
