...
uint32_t boot_param_checksum(const boot_param_t *param)
{
    ...
    return (uint32_t)boot_crc16_modbus((const uint8_t *)param,
                                       (uint32_t)(( size_t)__INTADDR__(&(((boot_param_t *)0)->checksum))));
}
...
_Bool boot_param_boot_flag_ok(uint8_t boot_flag)
{
    return (boot_flag == 0x00U) || (boot_flag == 0xA5U);
}
...
_Bool boot_param_save(boot_param_t *param)
{
    ...
    if (!ROM_erase_range(0x08010000U, 0x00001000U)) {
        return 0;
    }

    return ROM_buffer_write(0x08010000U, (const uint8_t *)param, sizeof(*param));
}
