#line 1 "..\\Function\\boot_upgrade.c"
...
#line 2 "..\\Function\\boot_upgrade.c"
...
#line 6 "..\\Function\\boot_upgrade.c"
...
#line 7 "..\\Function\\boot_upgrade.c"
...
static uint8_t g_upgrade_flash_buf[256U];

static _Bool boot_upgrade_flash_equal(uint32_t left, uint32_t right, uint32_t len)
{
    return memcmp((const void *)left, (const void *)right, len) == 0;
}

static _Bool boot_upgrade_copy_flash_range(uint32_t dst, uint32_t src, uint32_t len)
{
    uint32_t offset = 0U;
...
_Bool boot_upgrade_prepare_stage(boot_upgrade_ctx_t *ctx)
{
    ...
    if (!ROM_erase_range(0x08051000U, 0x00020000U)) {
        return 0;
    }
...
_Bool boot_upgrade_staged_image_ok(const boot_image_info_t *info)
{
    const uint8_t *magic = (const uint8_t *)0x08051000U;

    if ((info->size <= 4U) ||
        (info->size > 0x00020000U)) {
        return 0;
    }
    ...
    return boot_app_vector_ok(0x08051000U + 4U);
}
...
boot_status_t boot_upgrade_receive_stage_image(boot_upgrade_ctx_t *ctx)
{
    ...
    ret = raw_download_receive_image(0x08051000U, 0x00020000U, &ctx->pending_image);
...
_Bool boot_upgrade_restore_backup_to_app(void)
{
    ...
    if (!ROM_erase_range(0x08011000U, 0x00020000U)) {
        return 0;
    }

    if (!boot_upgrade_copy_flash_range(0x08011000U, 0x08031000U, 0x00020000U)) {
        return 0;
    }
...
_Bool boot_upgrade_backup_app(void)
{
    ...
    if (!ROM_erase_range(0x08031000U, 0x00020000U)) {
        return 0;
    }

    return boot_upgrade_copy_flash_range(0x08031000U, 0x08011000U, 0x00020000U);
}
...
boot_status_t boot_upgrade_execute(boot_upgrade_ctx_t *ctx, boot_param_t *param)
{
    ...
    image_size = ctx->pending_image.size - 4U;
    if ((image_size == 0U) || (image_size > 0x00020000U)) {
    ...
    if (!boot_upgrade_copy_flash_range(0x08011000U,
                                       0x08051000U + 4U,
                                       image_size) ||
        !boot_upgrade_flash_equal(0x08011000U,
                                  0x08051000U + 4U,
                                  image_size) ||
        !boot_app_can_boot()) {
    ...
    if (!boot_param_set_boot_flag(param, 0x00U)) {
        return BOOT_STATUS_FLASH;
    }
