...
)
{
    ...
    if(0 == drv){
        ...
        if(SD_OK == status){
            ...
            status = sd_bus_mode_config((((0xFFFFFFFFUL << (11)) & (0xFFFFFFFFUL >> (31U - (uint32_t)(12)))) & ((uint32_t)(1) << 11)));
        ...
        if(SD_OK == status){
            ...
            status = sd_transfer_mode_config( ((uint32_t)0x00000000) );
...
)
{
    ...
    if(0 == drv){
        if(1 == count){
            status = sd_block_read((uint32_t *)(&buff[0]), (uint32_t)(sector<<9), 512);
        }else{
            status = sd_multiblocks_read((uint32_t *)(&buff[0]), (uint32_t)(sector<<9), 512, (uint32_t)count);
        }
    }
    if(SD_OK == status){
        return RES_OK;
...
)
{
    ...
    if(0 == drv){
        if(1 == count){
            status = sd_block_write((uint32_t *)buff, sector<<9, 512);
        }else{
            status = sd_multiblocks_write((uint32_t *)buff, sector<<9, 512, (uint32_t)count);
        }
    }
    if(SD_OK == status){
        return RES_OK;
