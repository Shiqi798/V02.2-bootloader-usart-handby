...
int main(void)
{
    ...
    {
        if (gpio_input_bit_get(((((uint32_t)0x40020000U) + 0x00000000U) + 0x00001000U), ((uint32_t)((uint32_t)0x01U<<(15)))) == 0) 
        {
            enter_boot = 1;
            break;
        }
    ...
    rs485_printf("Waiting UART Command\r\n");
    while (1)
    {
        if (usart1_rx_flag == 1)
        {
			...
			strncpy(cmd_local, (char *)usart1_rx_buffer, sizeof(cmd_local)-1);
			cmd_local[sizeof(cmd_local)-1] = '\0';

			char *cmd_buf = cmd_local;
			...
			uint16_t len = strlen(cmd_buf);
			while (len>0 && (cmd_buf[len-1]=='\r'||cmd_buf[len-1]=='\n'||cmd_buf[len-1]==' '||cmd_buf[len-1]=='\t'))
				cmd_buf[--len] = '\0';

            
			if (strncmp(cmd_buf, "BOOT?", 5) == 0)
			{
				rs485_printf("\r\nBoot Mode: Bootloader\r\n");
				rs485_printf("Waiting UART Command\r\n");
			}
            else if (strstr(cmd_buf, "RUNAPP") != 0)
            {
                rs485_printf("\r\nJumping to APP...\r\n");
                USART1_ClearRxBuf();
                JumpToApp();
            }
            else if (strstr(cmd_buf, "RESET") != 0)
            {
                NVIC_SystemReset();
            }
            else
            {
                rs485_printf("[ERROR] Unknown Command\r\n");
            }
...
void JumpToApp(void)
{
    uint32_t app_addr = 0x08008000U;
    __disable_irq();
    
    ((SysTick_Type *) ((0xE000E000UL) + 0x0010UL) )->CTRL = 0;
    ((SysTick_Type *) ((0xE000E000UL) + 0x0010UL) )->LOAD = 0;
    ((SysTick_Type *) ((0xE000E000UL) + 0x0010UL) )->VAL  = 0;
