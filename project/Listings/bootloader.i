...
uint8_t appbin_read_write(void) {
    ...
    uint8_t buf[512u];   
    UINT br;                 
    uint32_t flash_addr = 0x08008000U;

    char *filename = "0:/app.bin";
    res = f_open(&b_fdst,filename, 0x01);
    flash_erase_app();
    for(uint32_t j = 0; j < 4 * 16; j++) 
    {
        
        res = f_read(&b_fdst, buf, 512u, &br);
        if(res != FR_OK || br == 0) break; 

        flash_write_app(flash_addr, buf, 512u); 
        flash_addr += 512u; 
    }
    f_close(&b_fdst);
    return 0;
...
{
    fmc_unlock();
    if(fmc_sector_erase((((0xFFFFFFFFUL << (3)) & (0xFFFFFFFFUL >> (31U - (uint32_t)(7)))) & ((uint32_t)(2))<< 3)) != FMC_READY) 
    {
        fmc_lock();
        return 1;
    }
    if(fmc_sector_erase((((0xFFFFFFFFUL << (3)) & (0xFFFFFFFFUL >> (31U - (uint32_t)(7)))) & ((uint32_t)(3))<< 3)) != FMC_READY)
    {
        fmc_lock();
        return 1;
    }
    if(fmc_sector_erase((((0xFFFFFFFFUL << (3)) & (0xFFFFFFFFUL >> (31U - (uint32_t)(7)))) & ((uint32_t)(4))<< 3)) != FMC_READY)
    {
        fmc_lock();
        return 1;
    }
...
uint8_t flash_erase_app(void)
 {
    uint8_t result = flash_erase_nsector(0x08008000U, 4); 
    return result;
 }
...
uint8_t flash_write_app(uint32_t saddr, uint8_t* data, uint32_t size)
{
    ...
    for(uint32_t i=0; i<size; i++) {
        addr = saddr + i;
         if(fmc_byte_program(addr, data[i]) != FMC_READY)
         {
             fmc_lock();
             return 1;
         }
